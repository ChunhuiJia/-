...
class MpcCar {
 private:
  ...
  // 通过龙格-库塔四阶计算得到input输入在dt时间后的状态的变化
  ...
  VectorX compensateDelay(const VectorX& x0) {
    ...
    double dt = 0.001;
    for (double t = delay_; t > 0; t -= dt) {  // delay_:0.125s
      int i = std::ceil(t / dt_);
      VectorU input = historyInput_[history_length_ - i];  // delay_/dt
 ...
 public:
  ...
  int solveQP(const VectorX& x0_observe) {
    ...
    lu_.coeffRef(2, 0) = predictInput_.front()(1) - ddelta_max_ * dt_;  // 限制前轮转角的转动速度
